...
final class EffectPortalAnimator
{
			...
			BooleanSupplier audible)
	{
		...
		{
			world.playSound(center, Sound.BLOCK_ENDER_CHEST_CLOSE, SoundCategory.BLOCKS, Settings.portalSoundVolume(0.36f), 0.55f);
		}
		if(!Settings.ENABLE_PARTICLES)
		{
			if(audible.getAsBoolean())
			{
				world.playSound(center, Sound.BLOCK_GLASS_BREAK, SoundCategory.BLOCKS, Settings.portalSoundVolume(0.3f), 0.9f);
			}
			return;
		}
		...
		holder[0] = () ->
		{
			...
			if(t >= CLOSE_CRACK_TICKS)
			{
				...
				if(audible.getAsBoolean())
				{
					world.playSound(center, Sound.BLOCK_GLASS_BREAK, SoundCategory.BLOCKS, Settings.portalSoundVolume(0.54f), 0.8f);
					world.playSound(center, Sound.BLOCK_GLASS_BREAK, SoundCategory.BLOCKS, Settings.portalSoundVolume(0.33f), 1.05f);
					world.playSound(center, Sound.ENTITY_ITEM_BREAK, SoundCategory.BLOCKS, Settings.portalSoundVolume(0.36f), 0.8f);
				}
				return;
			}
			...
			{
				world.playSound(center, Sound.BLOCK_GLASS_BREAK, SoundCategory.BLOCKS, Settings.portalSoundVolume(0.12f), 1.7f);
			}
			if(t % 3 == 1 && audible.getAsBoolean())
			{
				world.playSound(center, Sound.BLOCK_AMETHYST_BLOCK_HIT, SoundCategory.BLOCKS, Settings.portalSoundVolume(0.15f), 0.7f);
			}
